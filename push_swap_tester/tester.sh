...
# Usage: ./push_swap_tester.sh [directory to push_swap and checker_Mac progs ] [stacksize 0R stacksize_min-stacksize_max ] [nb_of_test per stacksize]
...
if ! [[ -f "$1/push_swap" ]]; then
	...
	printf "${RED}error: could not find checker_Mac in $1 \n${NOCOLOR}" >&2
	exit -1;
elif ! [[ -x "$1/push_swap" ]]; then
	printf "${RED}error: cannot execute push_swap in $1 \n${NOCOLOR}" >&2
	exit -1;
elif ! [[ -x "$1/checker_Mac" ]]; then
	printf "${RED}error: cannot execute checker_Mac in $1 \n${NOCOLOR}" >&2
	exit -1;
else
